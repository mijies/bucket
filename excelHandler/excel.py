...
def is_writable(func):
    def wrapper(*args):
        if not args[0].writable:
            raise NotImplementedError('Writable methods not allowed in read-only mode')
        func(*args)
...
class ExcelHandle:
    ...
    # return a list of target cell values by generator
    def iterate_row_values(self, sheetname, rows, cols, func):
        sheet = self.book[sheetname]
        for col in cols:
